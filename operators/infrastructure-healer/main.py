...
class InfrastructureHealer:
        ...
        for namespace in namespaces:
            try:
                ...
                for deployment in deployments.items:
                    action = await self.evaluate_deployment_scaling(deployment, scaling_config, rule)
                    if action:
    ...
    async def evaluate_deployment_scaling(self, deployment, scaling_config: Dict, rule: Dict) -> Optional[HealingAction]:
    ...
    async def evaluate_pod_health(self, rule: Dict) -> List[HealingAction]:
        ...
        for namespace in namespaces:
            try:
                ...
                for pod in pods.items:
                    action = await self.evaluate_single_pod_health(pod, health_config, rule)
                    if action:
                        actions.append(action)
    ...
    async def evaluate_single_pod_health(self, pod, health_config: Dict, rule: Dict) -> Optional[HealingAction]:
    ...
    async def execute_healing_action(self, action: HealingAction) -> bool:
        ...
        if action.dry_run or self.dry_run_mode:
            logger.info(f"DRY RUN: Would execute {action.action_type} on {action.target_resource}")
        ...
        try:
            with HEALING_DURATION.labels(action_type=action.action_type).time():
                ...
                if action.action_type == 'scale_up' or action.action_type == 'scale_down':
        ...
        try:
            ...
            for container in containers:
                if container.name == container_name:
                    ...
                    # Simple memory increase logic
                    if 'Mi' in current_memory:
                        memory_mb = int(current_memory.replace('Mi', ''))
                        new_memory = f"{memory_mb * 2}Mi"
                    else:
                        new_memory = '512Mi'  # Default increase
